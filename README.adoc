...
If you have a single-node OpenShift cluster, such as Minishift or Red Hat Container Development Kit, installed and running, you can deploy your example application there. A single-node OpenShift cluster provides you with access to a local cloud environment.
...
----
$ git clone git@github.com:geored/subtle-leather
...
You can interact with the example application using the form at the secured endpoint URL and the link:https://access.redhat.com/documentation/en-us/red_hat_build_of_eclipse_vert.x/#rhsso-realm-model_secured[provided users].
...
To run the integration test on the Secured HTTP API example application follow the steps in the link:https://launcher.fabric8.io/docs/vertx-runtime.html#running-the-vertx-secured-example-application-integration-tests_secured[Secured Example Application documentation].
